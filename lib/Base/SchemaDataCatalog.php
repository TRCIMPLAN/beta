<?php
/**
 * Plataforma de Conocimiento - Schema DataCatalog
 *
...
/**
 * Clase SchemaDataCatalog
 */
class SchemaDataCatalog extends SchemaCreativeWork {
    ...
    public function html() {
        ...
        if (is_object($this->dataset) && ($this->dataset instanceof SchemaDataset)) {
            $this->dataset->onTypeProperty = 'dataset';
            $this->dataset->identation     = $this->identation + 1;
            $this->dataset->is_article     = FALSE;
            $a[]                           = $this->dataset->html();
        }
        $a[] = $this->content_html();
